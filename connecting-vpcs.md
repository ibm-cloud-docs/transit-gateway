...
1. Add connections to your transit gateway now, or after it has been provisioned. {: #add-connection}
   ...
   1. Select the network connection to be attached to the transit gateway. To add connections later, see [Adding a connection](/docs/transit-gateway?topic=transit-gateway-adding-connections).
      ...
      * **{{site.data.keyword.powerSys_notm}}** - Creates a network connection to a {{site.data.keyword.powerSys_notm}} workspace to access the resources in a {{site.data.keyword.powerSys_notm}} colocation.
   ...
   1. Complete base network information (different depending on selected network connection) and choose a connection reach option:
      ...
      * **Add new connection in this account** - Enter a connection name and any other required information for your connection.
         ...
         * For **VPN gateway**, first select the region where your VPN gateway is deployed. Then, select from the available VPN gateways that support dynamic routing.
            ...
            For planning considerations about VPN gateway connections, including zone behavior, see [VPN gateway connection considerations](/docs/transit-gateway?topic=transit-gateway-helpful-tips#vpn-connection-considerations).
            {: attention}
